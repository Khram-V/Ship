...
begin
   ...
   if N<>0 then Delete(Input,1,N);
   N:=0;
   for I:=Length(input) downto 1 do begin
      if Input[I]=#32 then inc(N) else break;
   end;
   if N<>0 then Delete(Input,Length(Input)-N+1,N);
   Result:=input;
end;

procedure ProcessString(Input:String;var output:TStringList);
var Index   : Integer;
    Tmp     : String;
begin      // split a string value into multiple words
   Output.Clear;
   if Input='' then exit;
   repeat
      Index:=Pos(#10,Input);
...
begin
   ...
   if not Done then begin
      ...
      if not done then ShowMessage(Userstring(115));
   end;
end;
...
begin
   ...
   ToDo.Add(Source);
   I:=1;
   while I<=ToDo.Count do begin
      Current:=ToDo[I-1];
      Line:=0;
      While Line<Current.Count do begin
   ...
   if FileExistsUTF8( Filename ) then begin
      ...
      if not EOF(FFile) then begin
         ...
         if pos('#VRML V1.0 ASCII',Str)<>0 then begin
            ...
            FFileType:=ftVRML2;     //ValidFile:=true;
         end;
      end;
      ...
      while not EOF(FFile) do begin
         ...
         repeat                              // replace double spaces by only 1
            I:=Pos('  ',Str);
            if I<>0 then begin
      ...
      if Strings.Count>0 then begin
         ...
         While LineNr<Strings.Count do begin
            LoadNextObject(Strings,LineNr,NObj,Objectname,Words); //showmessage(words.Text);
            VRMLObject:=nil;
