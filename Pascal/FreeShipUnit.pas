...
private
   ...
   FCurvatureScale               : Real;     // Scalefactor used to increase or decrease the size of the curvature plot
...
function FindWaterViscosity(Density:Real;Units:TFreeUnitType):Real;
...
var I: Integer; TmpArea: Real; TmpCOG,MomI: Vector;
   ...
   begin
      ...
      if Spline.IntersectPlane(Plane,IntersectionData) then begin
         ...
         for I:=1 to Intersectiondata.NumberOfIntersections do begin
...
constructor TFreeFlowline.Create(Owner:TFreeShip);
      begin inherited Create;
            FOwner:=owner;
            FFlowLine:=TFreespline.Create; Clear;
      end;
...
begin
   ...
   if (FFlowline.nS>0) and (Viewport.ViewportMode=vmWireframe) then begin
      ...
      then P3D.Y:=-P3D.Y;
      Pt:=Viewport.Project(P3D);
      Size:=Round( Sqrt(Viewport.Zoom)*(Owner.Preferences.PointSize+1)/2 );
...
var Points,Faces: TFasterList;
    ...
    begin
       ...
       P2:=ProjectPointOnPlane(P1+50*Direction,Triangle.Plane);
   ...
   if Faces.Count>0 then begin
      ...
      if index<>-1 then begin //FFlowline.Add(Intersection); // trace triangles from here
         ...
         repeat
            ...
            if Valid
            then FFlowline.Add(Intersection)
            else Valid:=ProcessTriangle(Triangles[index],Skip1,Skip2,Intersection,Direction,Index);
            inc(Iteration);
...
procedure TFreeVisibility.FSetCurvatureScale(Val:Real);
var I:Integer;
begin
...
procedure TFreeVisibility.FSetCursorIncrement(val:Real);
begin
...
// try
      ...
      for I:=Ship.FUndoObjects.Count downto Ship.FUndoPosition+1
          do Ship.UndoObject[I-1].Delete; // Temp. set to the latest fileversion so that no data will be lost
      Ship.FFileVersion:=Currentversion;  // Temp. disable saving of preview image
      Ship.ProjectSettings.SavePreview:=False;
      UndoObject.FFileChanged:=Ship.FileChanged;
      UndoObject.FFileName:=Ship.Filename;
...
begin
   ...
   begin
      ...
      if Edges.Count>0 then begin
         Ship.Surface.ExtrudeEdges(Edges,Ve); // New edges are returned in the edges-list, select them
...
begin
  ...
  //
  for I:=Ship.Surface.NumberOfControlFaces-1 downto 0 do begin
    isPlus:=false; isMinus:=false; isZero:=false;
    Face:=Ship.Surface.ControlFace[I];
    if face.NumberOfpoints>2 then                   // точки и линии удаляются
    for J:=0 to Face.NumberOfpoints-1 do begin P3D:=Face.Point[J].Coordinate;
  ...
  if (RemovedF+RemovedP)>0 then begin Str:='';
    ...
    Ship.Build:=False;
    Ship.RebuildModel;
    if Assigned( Ship.OnUpdateGeometryInfo ) then
                 Ship.OnUpdateGeometryInfo( self );
  end else begin
...
var Flowline: TFreeFlowline; Undo: TFreeUndoObject;
begin
...
procedure TFreeEdit.Model_Scale(ScaleVector:Vector;OverrideLock,AdjustMarkers:Boolean);
var I,J   : integer;
    Point : TFreeSubdivisionControlPoint;
    P     : Vector;
    Marker: TFreeMarker;
...
begin
   ...
   For I:=Ship.NumberOfSelectedControlPoints downto 1 do begin
...
procedure TFreeShip.Extents( Var Min,Max:Vector );
...
function TFreeShip.FindLowestHydrostaticsPoint:Real;
var I,J     : Integer;
    First   : Boolean;
    Layer   : TFreeSubdivisionLayer;
begin
...
procedure TFreeShip.SubmergedHullExtents(Wlplane:Plate;var Min,Max:Vector);
var I,J,K,L    : Integer;
    FirstPoint : boolean;
    Layer      : TFreeSubdivisionLayer;
    Face       : TFreeSubdivisionControlFace;
