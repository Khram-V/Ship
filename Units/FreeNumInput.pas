...
interface uses
     SysUtils,Windows,  // WinTypes,WinProcs,
     Messages,
     Classes,
     Graphics,
     Controls,
...
var Val : extended;
begin
   if Text<>'' then begin Val:=StrToFloat(Text); end else Val:=0;
   if val<>FValue then begin FValue:=val; end;
   Result:=FValue;
end;

procedure TFreeNumInput.SetTabOnEnterKey(Value: Boolean);
begin
   if FTabOnEnterKey<>Value then FTabOnEnterKey:=Value;
end;
...
begin
   Result:=true;
   if FValidate and ((Value<FMin) or (Value>FMax)) then begin
...
begin
   x:=0;
   if (key=#13) and (FTabOnEnterKey) then begin
      if Valid(FValue) then begin
         if Assigned(FOnBeforeSetValue) then FOnBeforeSetValue(self);
...
//    try
         ...
         Valid(FValue);
         FormatText;
         if Length(Text)>MaxLength then begin {Check Length}
            Text:=OldText;
            MessageBeep(0);
         end;
//    except
...
//    end;
   ...
   if Key in ['0'..'9', '-',DecimalSeparator, #8,#13] then begin
      if Key in [DecimalSeparator] then if FDataType=dtFloat then else Key:=#0;
   ...
   if Sellength>0 then begin
      ...
      and (SelStart+SelLength >= pos(DecimalSeparator,Text)) then begin
   ...
   and (Length(Text)> MaxLength-FDecimals) then begin
      MessageBeep(0);
      Key:=#0;
      Exit;
   end;
   ...
   and (Length(Text)>FDecimals)then begin
      MessageBeep(0);
      Key:=#0;
      Exit;
   end;
   ...
   if (key='-') and (SelStart<>0)then begin
      MessageBeep(0);
      Key:=#0;
      Exit;
   end;
   ...
   begin
      ...
      if (Key in ['0'..'9']) and (FDecimals <>0) and (selstart=MaxLength-FDecimals-1)
          and (pos(DecimalSeparator,Text)=0) then begin
   ...
   and (selstart >= pos(DecimalSeparator,Text)) then begin
      MessageBeep(0);
      Key:=#0;
      Exit;
   end;
...
begin
  if FDataType=dtInteger then begin
...
begin
  MaxLength:=FDigits;
  if FDataType in [dtFloat] then begin
    X:=FValue;
