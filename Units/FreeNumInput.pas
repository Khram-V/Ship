...
begin FreeAndNil(FCanvas); inherited Destroy; end;
...
begin
   if Valid(Value) then begin
      ...
      FValue:=Value;
      if not FIsEditing then FInitialValue:=FValue;
      FormatText;
...
begin
   if FValidate<>Value then begin
      FValidate:=Value;
      if FValidate and ((FValue<=FMin) or (FValue>FMax)) then begin
         FValue:=FMin;
         FormatText;
         if Assigned(FOnChangeValue) then FOnChangeValue(self);
      end;
...
begin
   if FTabOnEnterKey<>Value then begin FTabOnEnterKey:=Value; end;
end;
...
//  if Key in ['0'..'9', '-',FormatSettings.DecimalSeparator, #8,#13] then begin
   ...
   if  (not (Key in [#8,#13])) and (pos(FormatSettings.DecimalSeparator,Text) >0)
        and (Length(Text)-pos(FormatSettings.DecimalSeparator,Text)>=FDecimals)
        and (selstart >= pos(FormatSettings.DecimalSeparator,Text)) then begin
      MessageBeep(0);
      Key:=#0;
      Exit;
   end;
...
{make sure the cursor is at the beginning if AutoSelect=False}
 if   (AutoSelect=False) then SelStart:=0;
...
var
  ...
  i:integer;
begin
//try {Using this to remove Currency symbol and Thousand seperator}
    TrashText:='';
...
procedure Register;
begin RegisterComponents ( 'FreeShip', [TFreeNumInput] );
end;
