...
procedure TFreeHullForm.SetCaption; begin Caption:=FCaptionText; end;
...
procedure TFreeHullForm.FormDestroy(Sender: TObject); begin end;
...
begin
   ...
   if Assigned(Owner) then
     for I:=0 to TMainForm(Owner).MDIChildCount-1 do begin
...
begin
   ...
   if (ssLeft in Shift) and (FAllowPanOrZoom) then begin
      // Zoom in or zoom out
     ...
     if Assigned(FFreeShip) then FFreeShip.MouseMove(Viewport,Shift,X,Y);
end;
...
begin
   ...
   begin if Freeship<>nil then Freeship.Edit.BackgroundImage_Delete(Viewport);
   end;
end;
