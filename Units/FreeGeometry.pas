...
TFreeAlphaBuffer= class // Alpha-buffer class used in the shading algorithm
private
...
TFreeZBuffer = class // Z-buffer class used in the shading algorithm
private
   FViewport : TFreeViewport;
...
TFreeBackgroundImage= class(TPersistent) // Background image properties for use in a viewport
private
...
TFreeViewport = class( TCustomPanel )       // TFlowPanel )
private
   FAngle,
...
// FDoubleBuffer     : Boolean;    // Double buffering prevents flickering when redrawing the viewport
   ...
   FScale,FZoom: TFloatType;    // Scale for projecting the 2D coordinates to the viewport
   ...
   FBackgroundImage  : TFreeBackgroundImage;                      // shade data
...
public
   ...
   procedure ShadeTriangle(P_1,P_2,P_3:T3DVector;C1,C2,C3:Extended);reintroduce;overload;
...
TFreeDevelopedPatch = class  // Unrolled Subdivision control face
Private
...
TFreeSpline = class(TFreeEntity)
   ...
   FNoPoints,           // Actual number of points present
...
TFreeSubdivisionLayer = class // color, visibility, symmetric, calc intersections/part of hull
private
...
public
   UnderWaterColor: TColor;       // Color used for shading the underwater part
...
begin
   ...
   Str:='';
   Index:=1;
   while Index<=Length(Source) do begin
      if Source[Index]<>#32 then begin
...
begin
   ...
   Val(Str,Result,Index);
// if Index<>0 then Raise Exception.Create('Invalid integer value in linenr. '+IntToStr(LineNr+1))
end;
...
//Function {TFreeBackgroundImage.}FOnChangeBackgroundImage( Sender: TObject ): TNotifyEvent;
...
begin                                                             // Add margin
   Diff:=(FMargin*0.01)*(Max-Min);        //  ScalePoint(0.01*FMargin,Max-Min);
   FMin3D:=Min-Diff;
   ...
   end else FScale:=1; // Decrease Scale with 1% to keep the model free from the edges
   FScale:=0.99*FScale;
...
begin
   if ShowDialog then begin
      ...
      Dialog.InitialDir:=Ship.Preferences.ExportDirectory;
...
begin
   ...
   if (BackgroundMode<>emNormal) and (ssRight in Shift) then begin
   ...
   end else if (BackgroundMode=emSetScale)
   ...
   if (BackgroundMode=emSetTransparentColor)
   and (ssLeft in Shift) then begin
      Pt:=Project(ZERO);
      if (Scale*Zoom*BackgroundImage.Scale)<>0 then begin
         Diff.X:=round((Pt.X-X)/(Scale*Zoom*BackgroundImage.Scale));
...
begin
   ...
   then begin                              // Background image moving procedure
      Scale:=1.0;
   ...
   if (Viewtype=fvPerspective)
   and ((ssLeft in shift) or (ssmiddle in shift))
   then begin                              // rotation using middle mousebutton
...
{  if (Focused)
   ...
   and (MousePos.Y>=0) and (MousePos.Y<Clientheight) then
}  begin
...
var Normal,{LSourceDirection,}Center: T3DVector;
...
// LSourceDirection:=Normalize( Center-FLight.Position );
...
begin                                         // Calculate data for the points
   ...
   LSourceDirection:=Center-FLight.Position;    // Calculate light vector
...
// Draw a smooth shaded triangle to the ZBuffer (used when all 3 corners of the triangle have
// different color, as for example when shading GAUSS curvature
procedure TFreeViewport.ShadeTriangle(P_1,P_2,P_3:T3DVector;C1,C2,C3:Extended);
var Left,Right,DLeft,dRight:TShadePoint;
    Y,d             : integer;
    Tmp : TShadePoint;
    V1,V2,V3:TShadePoint;
...
begin
   ...
   if (ShowSolid) or (ShowInteriorEdges) then begin
      ...
      for I:=1 to FDoneList.Count do begin
         Face:=FDoneList[I-1];
         if cap<>Face.NumberOfpoints then begin
   ...
   for J:=1 to FDoneList.Count do begin
      ...
      for K:=1 to Face.FPoints.Count do begin
         ...
         if ((Side1<-1e-5) and (Side2>1e-5)) or
            ((Side1>1e-5) and (Side2<-1e-5)) then begin
...
var I,J,K,N,BestIndex,ErrorIndex: Integer;
   ...
   var I,J,k,S,E,P,Index: Integer;
       ...
       Edge       : TFreeSubdivisionEdge;
       P1,P2      : TFreeSubdivisionPoint;
       Error,TotalError,_3DArea,_2DArea: extended;
       Temp       : Boolean;
   begin
      MaxError:=0.0;
      TotalError:=0.0;
...
begin
  ...
  if not Build then Rebuild;
  for I:=2 to NumberOfPoints-1 do begin
...
begin
   ...
   if N>0 then begin
      Min:=FRowknots[0];
      Max:=FRowknots[N-1];
      if Min<>Max then for I:=1 to N do begin
...
begin
   if FCurve.NumberOfPoints>1 then begin
      ...
      if not Owner.ShowControlNet and (Sel) then begin
      ...
      if Owner.DrawMirror then begin
         for I:=1 to FCurve.NumberOfPoints do begin
...
begin
   ...
   for I:=1 to NumberOfControlPoints do begin
   ...
   if NewPoints.Count>0 then begin
      ...
      while I<=NewPoints.Count do begin
         ...
         while J<=P1.NumberOfFaces do begin
            ...
            K:=1;
            Inserted:=False;
            while (K<=Face.NumberOfpoints) and (Not Inserted) do begin
...
var Layers        : TFasterList;
    ...
    NAssembled,I,J,K,Cols,Rows: Integer;
   ...
   if visible then begin
      ...
      if NAssembled>0 then begin           // assign all patches to new layers
         for I:=1 to NAssembled do begin
            ...
            and (AssFace.Faces[0][0].NumberOfPoints=4)) then begin
...
var I,Index,PrevIndex,NextIndex: Integer;
   ...
   function Angle_VV_3D(P1,P2,P3:T3DVector):extended;
   var V1X,V1Y,V1Z,
       V2X,V2Y,V2Z,L: extended;
   begin
      V1X:=P1.X-P2.X;
      V1Y:=P1.Y-P2.Y;
      V1Z:=P1.Z-P2.Z;
...
begin Result:=ZERO;
   for I:=1 to FFaces.Count do begin
      Face:=FFaces[I-1];
      if Face.NumberOfpoints>4 then begin // Face possibly concave at this point use the normal of ALL points from this face
         C:=Face.FaceCenter;
         N:=ZERO;
...
begin
   ...
   if (NumberOfFaces=5) and (NumberOfEdges=5) then begin // boundary of quad and triangle
      N:=0;
      for I:=1 to NumberOfFaces do if Face[I-1].NumberOfPoints=3 then inc(N);
      if (N=3) then Result:=True;
   end else if (NumberOfFaces=6) and (NumberOfEdges=6) then begin // regular point with all triangles
      N:=0;
      for I:=1 to NumberOfFaces do if Face[I-1].NumberOfPoints=3 then inc(N);
      if (N=6) then Result:=True;
   end else if (NumberOfFaces=4) and (NumberOfEdges=4) then begin // regular point with all quads
      Result:=True;
...
begin
   if VertexType in [svDart,svRegular] then begin
      ...
      P30:=nil;
      P33:=nil;
      for I:=1 to NumberOfEdges do begin
         Edge:=FEdges[I-1];
...
begin
   ...
   if VertexType in [svCrease] then begin                         //Faces:=nil;
      if Faces<>nil then begin N:=0;
         ...
         Result:=N=2;
         if (N=2) and (NumberOfFaces<>4) then Result:=true;
   ...
   or (FVertexType=svCorner) then Result:=FCoordinate else begin
      if FVertexType=svCrease then begin
         ...
         for I:=1 to FFaces.Count do begin
            Face:=FFaces[I-1];
            Center:=ZERO;
            if Face.NumberOfpoints=3 then begin inc(Nt); // Calculate centerpoint
               for j:=1 to Face.NumberOfpoints do begin
                  P:=Face.FPoints[J-1];
               ...
               Weight:=Pi/3;
            end else if Face.NumberOfpoints=4 then begin // Calculate centerpoint
               for j:=1 to Face.NumberOfpoints do begin
                  P:=Face.FPoints[J-1];
                  Weight:=1/4;
                  Center:=Center+Weight*P.FCoordinate;                          // Center.X:=Center.X+Weight*P.FCoordinate.X; Center.Y:=Center.Y+Weight*P.FCoordinate.Y; Center.Z:=Center.Z+Weight*P.FCoordinate.Z;
...
begin                                                            // FCoordinate
   ...
   FCoordinate.Z:=ReadFloatFromStr(LineNr,Str);                  // FVertexType
   if Str<>'' then begin
...
begin
   ...
   if (P.RegularPoint) and (P.VertexType<>svCorner) then begin // Find previous edge
      for I:=1 to P.NumberOfEdges do if P.Edge[I-1]<>self then begin
         Edge:=P.Edge[I-1];
...
var Face1,Face2,NewFace: TFreeSubdivisionControlFace;
    Edge: TFreeSubdivisionEdge;
    P1,P2: TFreeSubdivisionPoint;
    ...
    procedure Swap( var Ind1,Ind2:Integer );
      var Tmp:Integer; begin Tmp:=Ind1; Ind1:=Ind2; Ind2:=Tmp; end;
begin
   ...
   and (Face[1] is TFreeSubdivisionControlFace) then begin
      if (StartPoint.NumberOfEdges>2) and (EndPoint.NumberOfEdges>2) then begin
         ...
         if (Ind1=0) and (Ind2=Face1.NumberOfpoints-1) then Swap(Ind1,Ind2);
         if (Ind3=0) and (Ind4=Face2.NumberOfpoints-1) then Swap(Ind3,Ind4);
...
begin Result:=ZERO;
   if FPoints.Count>1 then begin
      for I:=1 to FPoints.Count do begin
         P:=FPoints[I-1];
         Result:=Result+P.FCoordinate;                                          // Result.X:=Result.X+P.FCoordinate.X; Result.Y:=Result.Y+P.FCoordinate.Y; Result.Z:=Result.Z+P.FCoordinate.Z;
      end;
      Result:=Result/FPoints.Count;                                             // Result.X:=Result.X/FPoints.Count; Result.Y:=Result.Y/FPoints.Count; Result.Z:=Result.Z/FPoints.Count;
   end;
end;
...
begin
   ...
   for I:=1 to NumberOfPoints do begin
      ...
      C:=C+P;                                                                   // C.X:=C.X+P.X; C.Y:=C.Y+P.Y; C.Z:=C.Z+P.Z;
   end;
   C:=C/NumberOfPoints;                         // calculate normal             // C.X:=C.X/NumberOfPoints; C.Y:=C.Y/NumberOfPoints; C.Z:=C.Z/NumberOfPoints;
   P1:=Point[NumberOfPoints-1];
   for I:=1 to NumberOfPoints do begin
      P2:=Point[I-1];
      N:=UnifiedNormal(C,P1.Coordinate,P2.Coordinate);
      Result:=Result+N;                                                         // Result.X:=Result.X+N.X; Result.Y:=Result.Y+N.Y; Result.Z:=Result.Z+N.Z;
      P1:=P2;
   end;
   Result:=Normalize( Result );
end;
...
var NewFace: TFreeSubdivisionFace;
    Index,J,I,TmpIndex: Integer;
    Pts: array[0..3] of TFreeSubdivisionPoint;
    PrevEdge,CurrEdge: TFreeSubdivisionEdge;
    P2,PrevEdgePoint,CurrEdgePoint,P2Point,NewLocation: TFreeSubdivisionPoint;
...
var I,J,K,Na,Nb,Capacity,Index: Integer;
    ...
    begin
      ...
      and (P.X<=Owner.MainframeLocation) then begin
         P.Y:=-P.Y;
         N.Y:=-N.Y;
      end;
      P1:=Viewport.Project(P);
      Viewport.PenColor:=color;
      V:=P+N;                                                                   // V.X:=P.X+N.X; V.Y:=P.Y+N.Y; V.Z:=P.Z+N.Z;
      ...
      if L>0 then begin
         ...
         N:=(Ldes/L)*N;                                                         // N.X:=(Ldes/L)*N.X; N.Y:=(Ldes/L)*N.Y; N.Z:=(Ldes/L)*N.Z;
         V:=P+N;                                                                // V.X:=P.X+N.X; V.Y:=P.Y+N.Y; V.Z:=P.Z+N.Z;
         P2:=Viewport.Project(V);
    ...
    procedure ZebraStripe(Camera:T3DVector;Point1,Point2,Point3:TFreeSubdivisionPoint;R,G,B,Zr,Zg,Zb:Byte);
    const Width=0.02;
           ...
           if (H-L)>0 then begin
              ...
              for I:=L to H do begin
                 ...
                 if (Val>PrevDp) and (Val<Dp) then begin
                  ...
                  P3D:=PrevP+T*(P-PrevP);                                       // P3D.X:=PrevP.X+T*(P.X-PrevP.X); P3D.Y:=PrevP.Y+T*(P.Y-PrevP.Y); P3D.Z:=PrevP.Z+T*(P.Z-PrevP.Z);
                  Add(P3D,Val);
                 end;
              end;
              Add(P,Dp);
...
begin
   if Viewport.ViewportMode<>vmWireframe then begin
      ...
      if (Owner.ShadeUnderWater)
      and (Viewport.ViewportMode=vmShade)
      and (Layer.UseInHydrostatics)then begin // Clip all triangles against the waterline plane
      ...
      if (Selected) and (Owner.ShowNormals) then begin
         // Draw nornals
         // First assemble all points within this controlface
         Points:=TFasterList.Create;
         for I:=1 to ChildCount do begin
            Child:=Self.Child[I-1];
            for J:=1 to Child.NumberOfpoints-1 do begin
...
begin
   ...
   if NumberOfpoints=4 then begin     // create one polymesh for all childfaces
      FaceData.NCols:=1;
      FaceData.NRows:=1;
...
begin
   ...
   for I:=1 to Layer.Count do begin
...
begin Result:=NIL;                                 Result:=NewPoint(P); exit;
   ...
   for I:=1 to NumberOfControlEdges do begin
      ...
      if Edge.FFaces.Count<=1 then begin                       // boundary edge
         if P=Edge.FStartpoint.FCoordinate then begin                        // if Sqr(P-Edge.FStartpoint.FCoordinate)<=MaxError then begin
            Result:=Edge.FStartpoint as TFreeSubdivisionControlPoint;
            break;
         end else
         if P=Edge.FEndpoint.FCoordinate then begin                          // if Sqr(P-Edge.FEndpoint.FCoordinate)<=MaxError then begin
   ...
   if Result=nil then begin               // Search controlpoints without edges
      for I:=1 to FControlPoints.Count do begin
         Point:=ControlPoint[I-1];
         if Point.NumberOfEdges=0 then begin
            if P=Point.FCoordinate then begin                                // if Sqr(P-Point.FCoordinate)<=MaxError then begin
               Result:=Point;
               break;
            end;
         end;
...
var ToDoList   : TFasterlist;
    ...
    { Type TPointRow=record NPoints:Integer; Points: array of TFreeSubdivisionPoint; end;
    ...
    var I,J,Index,NCols,NRows: Integer;
        ...
        begin
           ...
           while (I<=CtrlFaces.Count) and (Result=nil) do begin
              Face:=Ctrlfaces[I-1];
              J:=1;
              while (J<=Face.NumberOfpoints) and (Result=nil) do begin
                 P:=Face.Point[J-1];
                 NFaces:=1;
        ...
        begin
           if NAssembled=Capacity then begin
              inc(Capacity,50);
        ...
        var SearchBottom,SearchTop,SearchLeft,SearchRight,DataOK: Boolean;
            ...
            var I,J,N,Index: Integer; Tmp: TFreeSubdivisionFace;
            begin
               Result:=False;
               if Face.NumberOfpoints=4 then begin
            ...
            and (Faces.Count>0) do begin inc(Counter);
              ...
              if (Counter=1) and (SearchBottom) then begin
                 ...
                 for I:=2 to Cols do begin
                    ...
                    for J:=1 to Edge1.NumberOfFaces do begin
                       Face:=Edge1.Face[J-1];
...
begin
 { if (Index>=0) and (Index<Flayers.Count) then } Result:=FLayers[index];
...
begin
   if Val<>FShowControlNet then FShowControlNet:=Val;
end;
...
begin                                                   // Remove double points
   ...
   if NoPoints>2 then begin
      ...
      if (Result.NumberOfpoints<3) or (InvalidFace) then begin // Delete invalid controlfaces
         for J:=1 to Result.NumberOfpoints do begin
...
function TFreeSubdivisionSurface.AddControlFace
    ( Points:TFasterList; CheckEdges:Boolean; Layer:TFreeSubdivisionLayer
    ):TFreeSubdivisionControlFace;
...
begin
   ...
   for I:=1 to NumberOfControlCurves do ControlCurve[I-1].Destroy; FControlCurves.Clear;
...
begin
   ...
   for I:=1 to FEdges.Count do Edge[I-1].Destroy;   FEdges.Clear;
   for I:=1 to FPoints.Count do Point[I-1].Destroy; FPoints.Clear;
   for I:=1 to NumberOfControlFaces do Controlface[I-1].FControlEdges.Clear;
end;
           ...
           do begin
              ...
              if (Counter=1) and (SearchBottom) then begin
                 ...
                 for I:=2 to Cols do begin
                    ...
                    if Edge<>nil then for J:=1 to Edge.NumberOfFaces do begin
                       Face:=Edge.Face[J-1];
                       if ValidFace(Face) then begin
                          TmpFaces[NFaces]:=Face;
                          inc(NFaces);
                 ...
                 for I:=2 to Rows do begin
                    ...
                    if Edge<>nil then
                    for J:=1 to Edge.NumberOfFaces do begin
                       Face:=Edge.Face[J-1];
                       if ValidFace(Face) then begin
                          TmpFaces[NFaces]:=Face;
                          inc(NFaces);
...
begin
//   CPoints:=TFreeSubdivisionControlPoint.Create;
//   FacePoints:=TFreeSubdivisionControlPoint.Create;
...
//                           +#32+FloatToStrF(P.Coordinate.x,ffFixed,7,4));
      ...
      for I:=1 to NumberOfControlFaces do begin
         ...
         if CFace.Layer.Visible then begin                          // portside
            ...
            Strings.Add(Str);
            if (CFace.Layer.Symmetric) and (DrawMirror) then begin
               // Starboard side
               Str:='f';
...
begin
   ...
   for I:=1 to Self.FEdges.Count do begin
      Edge:=FEdges[I-1];
      if Edge.Crease then begin
         SourceList.Add(Edge);
      end;
...
begin
   ...
   LockedPoints:=0;
   for I:=1 to NumberOfSelectedControlFaces do begin
   ...
   for I:=1 to NumberOfSelectedControlEdges do begin
   ...
   for I:=1 to NumberOfSelectedControlPoints do begin
...
begin                                                 // Read layer information
   ...
   for I:=1 to N do begin
      ...
      for J:=1 to Np do begin
         Index:=ReadIntFromStr(LineNr,Str); // Attach controlface to controlpoints
      ...
      for J:=1 to Face.FPoints.Count do begin
         ...
         if Edge<>nil then Edge.AddFace(Face) else begin
           ...
           Edge.Crease:=True;
         end;                                                                   //#//#// else MessageDlg(Userstring(201)+'!',mtError,[mbOk],0);
         P1:=P2;
      end;
      Index:=ReadIntFromStr(LineNr,Str);                     // Read Layerindex
      While Index>NumberOfLayers-1 do AddNewLayer;
      Layer:=FLayers[Index];
...
begin
   I:=1;
   Points:=TFasterList.Create;
   while I<=NumberOfControlEdges do begin
...
begin
   ...
   if self.NumberOfControlFaces>0 then begin
      for I:=1 to NumberOfControlCurves do begin
         Curve:=ControlCurve[I-1];
         if FCurrentSubdivisionLevel=0 then begin
            Curve.FBuild:=False;
   ...
   end else if NumberOfControlPoints>0 then begin
      for I:=1 to NumberOfControlPoints do begin
         if I=1 then begin
