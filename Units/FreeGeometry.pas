...
const                                                          //Foot = 0.3048;
  ...
  IncrementSize = 16;   // amount of points which is automaticly allocated extra memory for
...
//TFasterListTFreeSubdivisionCurve = TFasterList<TFreeSubdivisionCurve>;
  ...
  public
    Owner: TFreeViewport;
    ShowInView: TFreeViewType;
    Bitmap: TBitmap;
    Quality: byte;
    Scale: TFloatType;
    procedure AssignData( I:TGraphic; V:TFreeViewType; O:TPoint; S:TFloatType;
                          T:boolean; C:TColor; A,Q,L:byte; Z:boolean );
    { Image,View,Origin,Scale,Transp,TranspCol,Alpha,Quality,Tolerance,Quiet }
    procedure Clear;
...
{ FOwner: TFreeViewport;
  ...
  FQuality: byte;
  FBitmap: TBitmap;
  procedure AssignData( var Data: TFreeBackgroundImageData );
...
//  FCameraLocation: T3DVector; // Position of the camera, following from the field of view and the distance of the camera
    ...
    FBackgroundScaleIsMouseOnFrameSide: (fsNone,fsE,fsW,fsN,fsS); // shade data
...
//  FVertScrollbar: TScrollBar;
    ...
    FUpdating: boolean;
    FLastResizeWidth,FLastResizeHeight,
    FLastResizeClientWidth,FLastResizeClientHeight:integer;
  ...
  private
    FLightIntensity,FAmbientIntencity: Single;
    ...
    property OnKeyUp;
{
  ...
  private
    ...
    FDoneList: TFasterListTFreeSubdivisionFace; // List containing developed faces in chronological order
    FBoundaryEdges: TFasterListTFreeSubdivisionEdge; // All edges forming the boundary edges of the developed plate, sorted in the correct order
    ...
    FCorners: TFasterListTFreeSubdivisionPoint; // contains all cornerpoints for dimensioning reasons
  ...
  private
    ...
    function GetMin: T3DVector; virtual;
    function GetMax: T3DVector; virtual;
    procedure SetBuilt(Val: boolean); virtual;
  public
    Color: TColor;         // Color when drawing
    Penstyle: TPenStyle;   // Pen style for drawing the line
    PenWidth: byte;        // Pen thickness to use when drawing
    ...
    property Min: T3DVector  read GetMin;
    property Max: T3DVector  read GetMax;
  end;
  {--------------------------------------------------------------------}
  { 3D CSpline                                                         }
  ...
  TFreeSpline = class(TFreeEntity)
    ...
    ShowCurvature,
    ShowPoints: boolean;
    CurvatureScale: TFloatType; // scale factor used to increase or decrease the scale of the curvature plot
    CurvatureColor: TColor;     // Color used for draing the curvature plot
  ...
  public
    ...
    procedure AddKnuckle(P: T3DVector); // add a new point to the curve
    ...
    function Value(Parameter: extended): T3DVector; overload;
    function Value(Parameter: extended; var PointNo1, PointNo2: integer): T3DVector; overload;
  ...
  private
    FColCapacity,FRowCapacity: integer;
    FColDegree,FRowDegree: integer;
    FColKnots,FRowKnots: TFloatArray;
  ...
  private                                  // calc intersections/part of hull
    ...
    FDescription:AnsiString; // Description of the layer, used as user identification
    ...
    FPatches: TFasterListTFreeSubdivisionControlFace; // List containing all controlpatches
    FAlphaBlend: byte;
    function FGetColor: TColor;
    function FGetCount: integer;
    function FGetDXFLayername: AnsiString;
    function FGetName: AnsiString;
    ...
    procedure FSetName(Val: AnsiString);
  ...
  public
    ...
    procedure FindEdgeFaceIntersectionPoints( Layer: TFreeSubdivisionLayer; var NewEdge: TFreeSubdivisionControlEdge; var NewPoints: TFasterListTFreeSubdivisionControlPoint);
    procedure FindIntersectionPoints( Layer: TFreeSubdivisionLayer; var NewPoints: TFasterListTFreeSubdivisionControlPoint);
    procedure FindIntersectionControlPoints( Layer: TFreeSubdivisionLayer; var NewPoints: TFasterListTFreeSubdivisionControlPoint);
    ...
    property SurfaceVisible: boolean read FSurfaceVisible write FSetSurfaceVisible;
    property ControlNetVisible: boolean read FControlNetVisible write FSetControlNetVisible;
  end;
  {-------------------------}
...
//    procedure PrintDebug; override;
  ...
  public
    ...
    function LastAveraging: T3DVector;
//  function ReverseAveraging: T3DVector;
  ...
  public
    ...
    procedure LoadBinary(Source: TFreeFileBuffer);
    procedure LoadFromStream(var LineNr: integer; Strings: TStringList);
    procedure MoveBy( Viewport:TFreeViewPort; dX,dY,dZ: TFloatType; vAnchor:TFreeSubdivisionControlPoint);
    procedure SetCoordinate( ViewPort:TFreeViewPort; Val: T3DVector; vAnchor:TFreeSubdivisionControlPoint);
    procedure SaveBinary(Destination: TFreeFileBuffer);
    procedure SaveToStream(Strings: TStringList);
...
//    procedure PrintDebug; override;
  ...
  private
    FControlPoints: TFasterListTFreeSubdivisionControlPoint;
    FLocked: boolean;
    function CalculateCenterPoint: T3DVector;
...
//  FOnChangeActiveControlPoint: TNotifyEvent;
    ...
    InUnreferenceControlFace,
    InUnreferenceControlEdge,
  ...
  public
    ...
    function AddControlPoint(P: T3DVector; aTolerance:double=1e-6): TFreeSubdivisionControlPoint; overload; virtual;
...
//  procedure SetObjectSelected(aObject: TFreeNamedObject; aSelected: boolean);
    ...
    procedure AssembleFacesToPatches(Layers: TFasterListTFreeSubdivisionLayer; Mode: TFreeAssembleMode; var AssembledPatches: TFreeFaceArray; var NAssembled: integer);
...
//  procedure ReverseAverage;
    ...
    procedure SortEdges(Edges: TFasterListTFreeSubdivisionEdge); overload; virtual;
    procedure SortEdges(Edges: TFasterListTFreeSubdivisionEdge; var Points: TFasterListTFreeSubdivisionPoint); reintroduce; overload;
    procedure SubDivide;
    property ActiveLayer: TFreeSubdivisionLayer read FActiveLayer write SetActiveLayer;
    property ActiveControlPoint : TFreeSubdivisionControlPoint read FActiveControlPoint write SetActiveControlPoint;
    property ActiveControlEdge : TFreeSubdivisionControlEdge read FActiveControlEdge write SetActiveControlEdge;
    property ActiveControlFace : TFreeSubdivisionControlFace read FActiveControlFace write SetActiveControlFace;
    property ActiveControlCurve : TFreeSubdivisionControlCurve read FActiveControlCurve write SetActiveControlCurve;
    property Changed: boolean read FChanged write FChanged;
    property ControlPoint[index: integer]: TFreeSubdivisionControlPoint read FGetControlpoint;
    property ControlPointGroup[index: integer]: TFreeSubdivisionControlPointGroup read FGetControlpointGroup;
    property ControlPoints: TFasterListTFreeSubdivisionControlPoint read FControlPoints;
    ...
    property ControlCurves: TFasterListTFreesubdivisionControlCurve read FControlCurves;
    ...
    property ControlEdges: TFasterListTFreeSubdivisionControlEdge read FCOntrolEdges;
    property ControlFace[index: integer]: TFreeSubdivisionControlFace read FGetControlFace;
    property ControlFaces: TFasterListTFreeSubdivisionControlFace read FCOntrolFaces;
    ...
    property ControlEdgeLineWidth: integer read FControlEdgeLineWidth write FControlEdgeLineWidth;
    property InternalEdgeLineWidth: integer read FInternalEdgeLineWidth write FInternalEdgeLineWidth;

    property DesiredSubdivisionLevel: byte read FDesiredSubdivisionLevel write SetDesiredSubdivisionLevel;
...
//  property FreeShip: TObject read FFreeShip write FFreeShip;
    ...
    property LayerColor: TColor read FLayerColor write FLayerColor; property LeakColor: TColor read FLeakColor write FLeakColor;
    property SplitSectionLocation: TFloatType read FSplitSectionLocation write FSplitSectionLocation;
    ...
    property NumberOfControlPointGroups: integer read FGetNumberOfControlPointGroups;
    property NumberOfFaces: integer read FGetNumberOfFaces;
...
//  property OnSelectItem: TNotifyEvent read GetOnSelectItemListener write SetOnSelectItemNotificationReceiver;
    ...
    property SubDivEdge[index: integer]: TFreeSubdivisionEdge read FGetEdge;
    ...
    property NumberOfSubDivEdges: integer read FGetNumberOfSubDivEdges;
    property NumberOfSubDivPoints: integer read FGetNumberOfSubDivPoints;
    property RebuildRequested: boolean read FRebuildRequested write FRebuildRequested;
    property ShadeUnderWater: boolean read FShadeUnderWater write SetShadeUnderWater;
...
//  property SelectedControlPoints: TFasterListTFreeSubdivisionControlPoint read FSelectedControlPoints;
    ...
    property ShowControlNet: boolean read FShowControlNet write SetFShowControlNet;
    property ShowFreeObjects: boolean read FShowFreeObjects write SetFShowFreeObjects;
    ...
    property SubdivisionMode: TFreeSubdivisionMode read FSubdivisionMode write SetSubdivisionMode;
    property UnderWaterColor: TColor read FUnderWaterColor write SetUnderwaterColor;
    property UnderWaterColorAlpha: byte read FUnderWaterColorAlpha write SetUnderwaterColorAlpha;
...
//function Subtract(AVec1, AVec2: T3DVector): T3DVector;    // subtract two vectors
...
begin
  ...
  Surface:=Owner;               //  for all subdivision points, edges and faces
  InUnreference:=false;
