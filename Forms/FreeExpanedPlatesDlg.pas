...
function TFreeExpanedplatesDialog.Execute
( FreeShip: TFreeShip;
  Plates: TFasterListTFreeDevelopedPatch ): boolean;
var
  I: integer;
  ...
  for I := 1 to FPlates.Count do begin
    ...
    if I = 1 then begin
      ...
      if Max.Y < -Min.Y then P2D.Y := Min.Y
                        else P2D.Y := -Max.Y; P2D.Y -=  0.5 * Clearance;
...
begin
  ...
  if Button = mbLeft then
    for I := FPlates.Count downto 1 do begin
      Active := FPlates[I - 1];
...
var
  I: integer; Patch: TFreeDevelopedPatch;
begin
  FFontSize:=SpinEditFontSize.Value;
  for I:=0 to FPlates.Count - 1 do begin
    Patch:=FPlates[I];
...
procedure TFreeExpanedplatesDialog.CloseDialogExecute(Sender: TObject); begin Close end;
