...
interface uses
  ...
  StdCtrls,  Buttons,
  Spin,      ExtCtrls,
  FreeShipUnit,FreeTypes,FreeGeometry,
  FreeHydrostaticsResultsDlg,FreeLanguageSupport;
type
  TFreeHydrostaticsForm = class(TForm)                { TFreeHydrostaticsForm }
    FloatSpinEdit1,FloatSpinEdit2,FloatSpinEdit3,FloatSpinEdit4: TFloatSpinEdit;
    Label1,Label2,Label3,Label4,Label9,_Label10,_Label11,_Label5: TLabel;
    Button1,Button2: TSpeedButton;
  ...
  private                                              { Private declarations }
    FFreeShip: TFreeShip;
    function FGetStartDraft: single; procedure FSetStartDraft(Val: single);
    function FGetEndDraft: single;   procedure FSetEndDraft(Val: single);
    function FGetDraftStep: single;  procedure FSetDraftStep(Val: single);
...
procedure TFreeHydrostaticsForm.Edit1KeyPress(Sender: TObject; var Key: char);
    begin if not (Key in [#8, '0'..'9', '.', '-', #13]) then key:=#0; end;
