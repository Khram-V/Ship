...
interface uses Windows, Math, Graphics, Dialogs, Forms,
...
procedure TFreeLinesplanFrame.ViewportRequestExtents(Sender: TObject;var Min, Max: T3DVector);
...
begin
   if Freeship<>nil then begin
      ...
      Min.X:=Min3D.X-0.5*space; // if no diagonals present in model, the mirror planview
      ...
      FAftOrigin.X:=Min.X+Tmp+0.5*FModelbeam;    // Attachpoint of aft bodyplan
      FAftOrigin.Y:=FProfileOrigin.Y-Space-FModelHeight;
      FAftOrigin.Z:=0.0;
      FFrontOrigin:=FAftOrigin;                // Attachpoint of front bodyplan
      FFrontOrigin.X:=Max.X-Tmp-0.5*FModelbeam;
      FPlanOrigin.X:=FProfileOrigin.X;             // Attachpoint for plan view
      FPlanOrigin.Y:=FAftOrigin.Y-space-0.5*FModelBeam+Min3D.Z;
      FPlanOrigin.Z:=0.0;
      Diff:=ScalePoint(0.03,Max3D-Min3D); Min:=Min-Diff; // Blowup the boundary box by 3%
      Diff:=ScalePoint(-1.0,Diff);        Max:=Max-Diff;
   end else begin
      Min.X:=-1;
      Min.Y:=Min.X;
      Min.Z:=Min.X;
...
var I,J,N,K,Steps,PenwidthFactor: Integer; Str: string;
   ...
   procedure DrawLineAtt(Attachpoint,P1,P2:T3DVector;Text:string;CenterText:Boolean);
   var Proj1,Proj2: T3DVector; Pts: array[0..1] of TPoint; W: Integer;
   begin
      ...
      if Text<>'' then begin
         if (CenterText) and (Pts[0].X=Pts[1].X) then begin
            ...
            Viewport.Canvas.TextOut(Pts[0].X-W,Pts[0].Y,Text);
            Viewport.Canvas.TextOut(Pts[1].X-W,Pts[1].Y,Text);
         end else begin
            Viewport.Canvas.TextOut(Pts[1].X,Pts[0].Y,Text);
            if (Pts[0].Y=Pts[1].Y) and (Pts[1].X<>Pts[0].X)
                then W:=Viewport.Canvas.TextWidth(Text) else W:=0;
            Viewport.Canvas.TextOut(Pts[0].X-W,Pts[1].Y,Text);
         end;
      end;
   end;{DrawLineAtt}
   ...
   begin
      ...
      if (lvAftBody in views) and (Spline.Max.X<=MainFrame) then begin
         for I:=0 to steps do begin
            P:=Spline.Value(I/steps);
      ...
      if (lvFrontBody in views) and (Spline.Min.X>=MainFrame) then begin
         for I:=0 to steps do begin
            P:=Spline.Value(I/steps);
...
// try
      if (not ShowMonochrome.Checked) and (ShowFillcolor.Checked) then begin
         ...
         // draw dwl in profile
         DrawLineAtt(FProfileOrigin,Vector(FMin3D.X-Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),Vector(FMax3D.X+Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),'',False);
         // draw dwl in aft bodyplan
         Space:=CalculateSpace(textspace,FMin3D.Y,FMax3D.Y);
         DrawLineAtt(FAftOrigin,Vector(-FMax3D.Y-Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),Vector(FMax3D.Y+Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),'',False);
         // draw dwl in front bodyplan
         DrawLineAtt(FFrontOrigin,Vector(-FMax3D.Y-Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),Vector(FMax3D.Y+Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),'',False);
      end;
      ...
      // draw baseline
      DrawLineAtt(FProfileOrigin,Vector(FMin3D.X-Space,FMin3D.Z,0),Vector(FMax3D.X+Space,FMin3D.Z,0),Userstring(184)+#32+
                  ConvertDimension(FMin3D.Z,Freeship.ProjectSettings.ProjectUnits),False);
      // draw dwl
      DrawLineAtt(FProfileOrigin,Vector(FMin3D.X-Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),Vector(FMax3D.X+Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),Userstring(185)+#32+
      ...
      begin
         ...
         DrawLineAtt(FProfileOrigin,Vector(FMin3D.X-Space,Tmp,0),Vector(FMax3D.X+Space,Tmp,0),Str,False);
      end;
      ...
      begin
         ...
         Str:=ConvertDimension(Tmp,Freeship.ProjectSettings.ProjectUnits);
         DrawLineAtt(FProfileOrigin,Vector(Tmp,FMin3D.Z-space,0),Vector(Tmp,FMax3D.Z+space,0),Str,True);
      end;
      ...
      // draw base
      DrawLineAtt(FAftOrigin,Vector(-FMax3D.Y-Space,FMin3D.Z,0),Vector(FMax3D.Y+Space,FMin3D.Z,0),Userstring(184)+#32+ConvertDimension(FMin3D.Z,Freeship.ProjectSettings.ProjectUnits),False);
      // draw dwl
      DrawLineAtt(FAftOrigin,Vector(-FMax3D.Y-Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),Vector(FMax3D.Y+Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),Userstring(185)+#32+
                  ConvertDimension(FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,Freeship.ProjectSettings.ProjectUnits),False);
      Space:=CalculateSpace(textspace,FMin3D.Z,FMax3D.Z);
      DrawLineAtt(FAftOrigin,Vector(0.0,FMin3D.Z-space,0),Vector(0.0,FMax3D.Z+space,0),Userstring(245),True);
      Space:=CalculateSpace(textspace,FMin3D.Y,FMax3D.Y);
      Viewport.FontColor:=clBlack;
      Viewport.SetPenWidth(PenwidthFactor);
      for I:=1 to FFreeship.NumberofWaterlines do
      begin
         Tmp:=-FFreeship.Waterline[I-1].Plane.D;
         Str:=ConvertDimension(Tmp,Freeship.ProjectSettings.ProjectUnits);
         DrawLineAtt(FAftOrigin,Vector(-FMax3D.Y-Space,Tmp,0),Vector(FMax3D.Y+Space,Tmp,0),Str,False);
      end;
      Space:=CalculateSpace(textspace,FMin3D.Z,FMax3D.Z);
      ...
      begin
         ...
         Str:=ConvertDimension(Tmp,Freeship.ProjectSettings.ProjectUnits);
         DrawLineAtt(FAftOrigin,Vector(Tmp,FMin3D.Z-space,0),Vector(Tmp,FMax3D.Z+space,0),Str,True);
         DrawLineAtt(FAftOrigin,Vector(-Tmp,FMin3D.Z-space,0),Vector(-Tmp,FMax3D.Z+space,0),Str,True);
      end;
      ...
      // draw baseline
      DrawLineAtt(FFrontOrigin,Vector(-FMax3D.Y-Space,FMin3D.Z,0),Vector(FMax3D.Y+Space,FMin3D.Z,0),Userstring(184)+#32+
                  ConvertDimension(FMin3D.Z,Freeship.ProjectSettings.ProjectUnits),False);
      // draw dwl
      DrawLineAtt(FFrontOrigin,Vector(-FMax3D.Y-Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),Vector(FMax3D.Y+Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,0),Userstring(185)+#32+
                  ConvertDimension(FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft,Freeship.ProjectSettings.ProjectUnits),False);
      Space:=CalculateSpace(textspace,FMin3D.Z,FMax3D.Z);
      DrawLineAtt(FFrontOrigin,Vector(0.0,FMin3D.Z-space,0),Vector(0.0,FMax3D.Z+space,0),Userstring(245),True);
      Space:=CalculateSpace(textspace,FMin3D.Y,FMax3D.Y);
      Viewport.FontColor:=clBlack;
      Viewport.SetPenWidth(PenwidthFactor);
      for I:=1 to FFreeship.NumberofWaterlines do
      begin
         Tmp:=-FFreeship.Waterline[I-1].Plane.D;
         Str:=ConvertDimension(Tmp,Freeship.ProjectSettings.ProjectUnits);
         DrawLineAtt(FFrontOrigin,Vector(-FMax3D.Y-Space,Tmp,0),Vector(FMax3D.Y+Space,Tmp,0),Str,False);
      end;
      Space:=CalculateSpace(textspace,FMin3D.Z,FMax3D.Z);
      for I:=1 to FFreeship.NumberofButtocks do
      begin
         Tmp:=-FFreeship.Buttock[I-1].Plane.D;
         Str:=ConvertDimension(Tmp,Freeship.ProjectSettings.ProjectUnits);
         DrawLineAtt(FFrontOrigin,Vector(Tmp,FMin3D.Z-space,0),Vector(Tmp,FMax3D.Z+space,0),Str,True);
         DrawLineAtt(FFrontOrigin,Vector(-Tmp,FMin3D.Z-space,0),Vector(-Tmp,FMax3D.Z+space,0),Str,True);
      end;
      ...
      DrawLineAtt(FPlanOrigin,Vector(FMin3D.X-Space,0,0),Vector(FMax3D.X+Space,0,0),Userstring(245),False);
      Viewport.FontColor:=clBlack;
      Viewport.SetPenWidth(PenwidthFactor);
      for I:=1 to FFreeship.NumberofButtocks do
      begin
         Tmp:=-FFreeship.Buttock[I-1].Plane.D;
         Str:=ConvertDimension(Tmp,Freeship.ProjectSettings.ProjectUnits);
         DrawLineAtt(FPlanOrigin,Vector(FMin3D.X-Space,Tmp,0),Vector(FMax3D.X+Space,Tmp,0),Str,False);
         if (Freeship.NumberofDiagonals=0) and (MirrorPlanview.Checked) then DrawLineAtt(FPlanOrigin,Vector(FMin3D.X-Space,-Tmp,0),Vector(FMax3D.X+Space,-Tmp,0),Str,False);
      end;
      Space:=CalculateSpace(textspace,FMin3D.Y,FMax3D.Y);
      for I:=1 to FFreeship.NumberofStations do
      begin
         Tmp:=-FFreeship.Station[I-1].Plane.D;
         Str:=ConvertDimension(Tmp,Freeship.ProjectSettings.ProjectUnits);
         if (Freeship.NumberofDiagonals=0) and (MirrorPlanview.Checked) then DrawLineAtt(FPlanOrigin,Vector(Tmp,-FMax3D.Y-space,0),Vector(Tmp,FMax3D.Y+space,0),Str,True)
                                           else DrawLineAtt(FPlanOrigin,Vector(Tmp,-FDiagonalWidth-space,0),Vector(Tmp,FMax3D.Y+space,0),Str,True);
      end;
...
var SaveDialog: TSaveDialog;
   ...
   procedure WriteDXFPoint(N:integer;P:T3DVector);
       begin Strings.Add(IntToStr(N)+EOL+Truncate(P.X,6));
             Strings.Add(IntToStr(N+10)+EOL+Truncate(P.Y,6)); end;
...
begin
   ...
   if SaveDialog.Execute then begin
      ...
      // draw dwl
      AddLine(Vector(FMin3D.X-Space,0,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft),Vector(FMax3D.X+Space,0,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft),[lvProfile],Userstring(185),clRed);
      ...
      // draw dwl
      AddLine(Vector(0.0,-FMax3D.Y-Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft),Vector(0.0,FMax3D.Y+Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft),[lvAftBody],Userstring(185),clRed);
      for I:=1 to FFreeship.NumberofWaterlines do begin
         Tmp:=-FFreeship.Waterline[I-1].Plane.D;
      ...
      // draw dwl
      AddLine(Vector(0.0,-FMax3D.Y-Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft),Vector(0.0,FMax3D.Y+Space,FMin3D.Z+FFreeship.ProjectSettings.ProjectDraft),[lvFrontBody],Userstring(185),clRed);
      for I:=1 to FFreeship.NumberofWaterlines do begin
         Tmp:=-FFreeship.Waterline[I-1].Plane.D;
